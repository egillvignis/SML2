...
if __name__ == "__main__":
    ...
    # init constants
    omega = 0.5        # power decay of the learning rate
    ...
    for decks in [inf]:
        ...
        pl.plot_value_function(V_conv_filt_MC,
                               ...
                               directory = plot_dir,
                               file_name = "3D_exp_MC_" + str(decks) + "_decks.png", show = False)
        """
        """
    ...
    opt_No_Ace = dict()
    for key in keys:
        if key[2] == 0 and key[0] < 21:
            No_Ace.append(key)
            if sumQ[key][0] <=  sumQ[key][1]:
                opt_No_Ace[key] = 0
            else:
                opt_No_Ace[key] = 1
        elif key[2] == 1 and key[0] < 21:
            Ace.append(key)
            if sumQ[key][0] <= sumQ[key][1]:
                opt_Ace[key] = 0
            else:
    ...
    print(opt_Ace)
    mat_Ace = np.zeros((20,10))
    mat_No_Ace = np.zeros((20,10))
    mat_No_Ace -= 1
    mat_Ace -= 1
    print(mat_No_Ace)
    print(mat_Ace)
    ...
    for x in No_Ace:
        mat_No_Ace[x[0]-1,x[1]-1] = opt_No_Ace[x]

    for y in Ace:
        mat_Ace[y[0]-1,y[1]-1] = opt_Ace[y]
